...
#
# Copyright (c) 2014 Sophos Group. All rights reserved.
# Sophos, Sophos Anti-Virus, SAV, SAV Interface, SAVI,
# PureMessage, Sophos MailMonitor, Intercheck and Sweep
# are registered trademarks of Sophos Plc and Sophos Group.
...
WINDOWS64_PATH = '/current/win64/'
UNIX_PATH    = '/current/unix/'
MAC_PATH     = '/current/mac/'
DATA_PATH    = '/data/'
VDL_PACKAGE  = 'vdl.zip'
...
def download_engine(package, output, user, password):
    ...
    if re.search('win32', package):
        url = BASE_URL + WINDOWS_PATH + package
    elif re.search('win64', package):
        url = BASE_URL + WINDOWS64_PATH + package
    elif re.search('mac', package):
        url = BASE_URL + MAC_PATH + package
    else:
