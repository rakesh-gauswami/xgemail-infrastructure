...
data "aws_security_group" "warmup_delivery_ec2" {
  id = local.input_param_sg_warmup_delivery_ec2_id
}
...
resource "aws_security_group_rule" "ec2_ingress_outbound_delivery_ec2_smtp" {
  ...
  protocol                 = "tcp"
  security_group_id        = aws_security_group.security_group_ec2.id
  source_security_group_id = data.aws_security_group.warmup_delivery_ec2.id
}
