...
data "aws_security_group" "mf_outbound_delivery_ec2" {
  id = local.input_param_sg_mf_outbound_delivery_ec2_id
}
...
resource "aws_security_group_rule" "ec2_ingress_outbound_delivery_ec2_smtp" {
  type                     = "ingress"
  from_port                = local.smtp_tcp_port
  to_port                  = local.smtp_tcp_port
  protocol                 = "tcp"
  security_group_id        = aws_security_group.security_group_ec2.id
  source_security_group_id = data.aws_security_group.mf_outbound_delivery_ec2.id
}
