locals {
  ...
  input_param_sg_mf_inbound_delivery_ec2_id     = nonsensitive(data.aws_ssm_parameter.sg_mf_inbound_delivery_ec2_id.value)
...
data "aws_ssm_parameter" "sg_mf_inbound_delivery_ec2_id" {
  provider = aws.parameters

  name = "/central/sg/mf/inbound/delivery/ec2/id"
}
