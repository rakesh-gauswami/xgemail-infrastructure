locals {
  ...
  input_param_sg_outbound_delivery_ec2_id     = nonsensitive(data.aws_ssm_parameter.sg_outbound_delivery_ec2_id.value)
...
data "aws_ssm_parameter" "sg_outbound_delivery_ec2_id" {
  provider = aws.parameters

  name = "/central/sg/outbound/delivery/ec2/id"
}
