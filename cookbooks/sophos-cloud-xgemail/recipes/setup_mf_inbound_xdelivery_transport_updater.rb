...
PACKAGE_DIR                    = "#{XGEMAIL_FILES_DIR}/mf-inbound-xdelivery-transport"
TRANSPORT_UPDATER_SCRIPT       = 'mf-inbound.xdelivery.transport.updater.py.erb'
...
file '/etc/rsyslog.d/00-xgemail-transportupdater.conf' do
  content "if $syslogtag == '[mfix-transport-updater]' then /var/log/xgemail/transportupdater.log\n& ~"
  mode '0600'
  owner 'root'
  group 'root'
end
