...
class MessageHistoryTest(unittest.TestCase):
    ...
    def test_can_generate_mh_event_empty_message_context(self):
        raw_sqs_message = "{ \"schema_version\": 20170224, \"message_path\": \"messages/2020/11/17/17/bb9da2f8b7bc4e7594b4bf6cc1d91e992120a065dae6a4a89a33011f1f9866c2/172.20.0.150-4CbCqX0zJRzRhQm-o365.qa1.sasubr.com\", \"accepting_server_ip\": \"172.20.0.150\", \"queue_id\": \"4CbCqX0zJRzRhQm_UUID_a32958e5f18345e39096be8a077ed820\", \"akm_key\": \"not_supported\", \"nonce\": \"not_supported\", \"message_key\": \"not_supported\", \"message_path_type\": \"NORMAL\", \"submit_message_type\": \"INTERNET\", \"message_context\": { \"mh_meta_data\": {  } } }"
        sqs_message = self.parse_sqs_message(raw_sqs_message, "test")
        mail_info, can_generate_mh_event = messagehistory.get_mail_info(sqs_message, None, None)
        self.assertEqual(can_generate_mh_event, False)
        self.assertEqual(mail_info, None)
    ...
    def test_can_generate_mh_event_empty_mh_mail_info(self):
        raw_sqs_message = "{ \"schema_version\": 20170224, \"message_path\": \"messages/2020/11/17/17/bb9da2f8b7bc4e7594b4bf6cc1d91e992120a065dae6a4a89a33011f1f9866c2/172.20.0.150-4CbCqX0zJRzRhQm-o365.qa1.sasubr.com\", \"accepting_server_ip\": \"172.20.0.150\", \"queue_id\": \"4CbCqX0zJRzRhQm_UUID_a32958e5f18345e39096be8a077ed820\", \"akm_key\": \"not_supported\", \"nonce\": \"not_supported\", \"message_key\": \"not_supported\", \"message_path_type\": \"NORMAL\", \"submit_message_type\": \"INTERNET\", \"message_context\": { \"mh_meta_data\": {  \"mail_info\" : {} } } }"
        sqs_message = self.parse_sqs_message(raw_sqs_message, "test")
        mail_info, can_generate_mh_event = messagehistory.get_mail_info(sqs_message, None, None)
        self.assertEqual(can_generate_mh_event, False)
    ...
    def test_can_generate_mh_event_true(self):
        raw_sqs_message = "{ \"schema_version\": 20170224, \"message_path\": \"messages/2020/11/17/17/bb9da2f8b7bc4e7594b4bf6cc1d91e992120a065dae6a4a89a33011f1f9866c2/172.20.0.150-4CbCqX0zJRzRhQm-o365.qa1.sasubr.com\", \"accepting_server_ip\": \"172.20.0.150\", \"queue_id\": \"4CbCqX0zJRzRhQm_UUID_a32958e5f18345e39096be8a077ed820\", \"akm_key\": \"not_supported\", \"nonce\": \"not_supported\", \"message_key\": \"not_supported\", \"message_path_type\": \"NORMAL\", \"submit_message_type\": \"INTERNET\", \"message_context\":  { \"mh_meta_data\": { \"mail_info\" : { \"generate_mh_events\" : true} } } }"
        sqs_message = self.parse_sqs_message(raw_sqs_message, "test")
        mail_info, can_generate_mh_event = messagehistory.get_mail_info(sqs_message, None, None)
    ...
    def test_can_generate_mh_event_false(self):
        raw_sqs_message = "{ \"schema_version\": 20170224, \"message_path\": \"messages/2020/11/17/17/bb9da2f8b7bc4e7594b4bf6cc1d91e992120a065dae6a4a89a33011f1f9866c2/172.20.0.150-4CbCqX0zJRzRhQm-o365.qa1.sasubr.com\", \"accepting_server_ip\": \"172.20.0.150\", \"queue_id\": \"4CbCqX0zJRzRhQm_UUID_a32958e5f18345e39096be8a077ed820\", \"akm_key\": \"not_supported\", \"nonce\": \"not_supported\", \"message_key\": \"not_supported\", \"message_path_type\": \"NORMAL\", \"submit_message_type\": \"INTERNET\", \"message_context\": { \"mh_meta_data\": { \"mail_info\" : { \"generate_mh_events\" : false} } } }"
        sqs_message = self.parse_sqs_message(raw_sqs_message, "test")
        mail_info, can_generate_mh_event = messagehistory.get_mail_info(sqs_message, None, None)
        self.assertEqual(can_generate_mh_event, False)
    ...
    def test_update_accepted_event_outbound(self):
        ...
        sender = 'admin@senderdomain.com'
        mail_info = accept_events[sender]['mail_info']

        #Prepare metadata
        metadata = self.get_sample_metadata(mail_info)

        recipients = []
        recipients.append(sender) #To make sure we are not considering recipients when direction is outbound.
        ...
        self.assertFalse('s3_resource_id' in mail_info) #Because we used incorrect sender in above line.
        ...
        self.assertEqual(mail_info['s3_resource_id'], s3_file_path)
        self.assertEqual(mail_info['decorated_queue_id'], metadata.get_queue_id())
