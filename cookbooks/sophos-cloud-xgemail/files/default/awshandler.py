...
class AwsHandler(object):
  def __init__(self, aws_region):
    ...
    if station_account_role_arn == 'none':
      ...
      self.session_ttl = 3000
      self.session_name = uuid4().hex
      self.sts_arn = station_account_role_arn
      self.session = self.refreshable_session()
  ...
  def __get_session_credentials(self):
    """
        ...
        """
    session = self.session or Session(region_name=self.region_name)
